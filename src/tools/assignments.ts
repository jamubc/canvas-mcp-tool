...
export class AssignmentsTools {
  ...
  async listAssignments(params: z.infer<typeof ListAssignmentsSchema>): Promise<CanvasAssignment[]> {
    try {
      const cacheKey = `assignments:${params.course_id}:${JSON.stringify(params)}`;
      const cached = cache.get<CanvasAssignment[]>(cacheKey);
      ...
      const assignments = await this.client.getAllPages<CanvasAssignment>(
  ...
  async getAssignment(params: z.infer<typeof GetAssignmentSchema>): Promise<CanvasAssignment> {
    try {
  ...
  async submitAssignment(params: z.infer<typeof SubmitAssignmentSchema>): Promise<CanvasSubmission> {
