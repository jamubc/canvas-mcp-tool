...
export class UsersTools {
  ...
  async getProfile(params: z.infer<typeof GetProfileSchema>): Promise<CanvasUser> {
    try {
      const userId = params.user_id || 'self';
      const cacheKey = `user:profile:${userId}`;
      const cached = cache.get<CanvasUser>(cacheKey);
      ...
      const response = await this.client.get<CanvasUser>(`/users/${userId}/profile`);
  ...
  async listUsers(params: z.infer<typeof ListUsersSchema>): Promise<CanvasUser[]> {
    try {
      ...
      const users = await this.client.getAllPages<CanvasUser>(
        `/courses/${params.course_id}/users`,
        queryParams
      );
