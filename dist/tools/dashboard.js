...
export class DashboardTools {
    ...
    async getDashboardCards(_params) {
        try {
    ...
    async listCalendarEvents(params) {
        try {
            ...
            if (!params.context_codes || params.context_codes.length === 0) {
                const events = await this.client.getAllPages('/users/self/calendar_events', queryParams);
                cache.set(cacheKey, events, 180000); // Cache for 3 minutes
                logger.info(`Fetched ${events.length} calendar events`);
                return events;
            }
            else {
                const events = await this.client.getAllPages('/calendar_events', queryParams);
                cache.set(cacheKey, events, 180000); // Cache for 3 minutes
                logger.info(`Fetched ${events.length} calendar events`);
                return events;
            }
